...
// --- THE AHA! MOMENT: Seed Demo Data for New Users ---
app.post("/api/seed-demo", requireAuth, async (req, res) => {
    try {
        const userId = req.auth.userId;

        // 1. Create a Welcome Session
        const demoSession = await Session.create({
            sessionId: "sess_demo_" + Date.now(),
            owner: userId,
            title: "🏈 Welcome to Vantage Pro (Demo)",
            type: "team",
            sport: "football",
            history: [{ role: 'model', text: "Welcome to your War Room. I've pre-loaded a sample video analysis and a playbook concept for you to explore." }],
            roster: [
                { identifier: "#15", position: "QB", grade: "A", speed: 82, notes: ["Elite arm angles"], weaknesses: ["Drops eyes vs A-gap pressure"] },
                { identifier: "#87", position: "TE", grade: "A+", speed: 88, notes: ["Primary target on Mesh"], weaknesses: [] }
            ]
        });

        // 2. Create a pre-analyzed AI Scout Card
        await Clip.create({
            owner: userId,
            sessionId: demoSession.sessionId,
            sport: "football",
            title: "Demo: Red Zone Mesh",
            formation: "Bunch Right vs Cover 3",
            o_formation: "Bunch Right",
            d_formation: "Cover 3",
            section: "Inbox",
            videoUrl: "https://res.cloudinary.com/demo/video/upload/v1689617300/c_fill,h_400,w_600/sample_video.mp4", // Fallback placeholder video
            fullData: {
                title: "Demo: Red Zone Mesh",
                data: { o_formation: "Bunch Right", d_formation: "Cover 3", situation: { play_type: "pass", down: 3, distance_togo: 4, plot_startX: 50, plot_startY: 80, plot_endX: 50, plot_endY: 20 } },
                scouting_report: {
                    summary: "The offense runs a classic Mesh concept out of a Bunch set to beat the Cover 3 match principles. The 'Sit' route perfectly settles in the void left by the dropping Linebacker.",
                    report_card: { overall: "A-", football_iq: "A", technique: "B+", effort: 90 },
                    coaching_prescription: { fix: "The Z receiver needs to rub closer to the X receiver on the cross to legally pick the trailing DB.", drill: "Traffic Cone Rub Drill" }
                },
                positional: [
                    { group: "Quarterback", action: "Quick drop", analysis: "Great job keeping eyes downfield to manipulate the curl-flat defender." },
                    { group: "Receivers", action: "Mesh crossers", analysis: "Need to be tighter on the cross. Too much space allowed the DB to slip through." }
                ]
            }
        });

        // 3. Create a pre-drawn Playbook Concept
        await Play.create({
            owner: userId,
            playId: "play_demo_" + Date.now(),
            title: "Demo: Mesh Sit",
            linkedSessionId: demoSession.sessionId,
            elements: [
                { id: "c", type: "O", label: "C", x: 600, y: 600 },
                { id: "qb", type: "O", label: "Q", x: 600, y: 640 },
                { id: "wr1", type: "O", label: "X", x: 400, y: 600 },
                { id: "wr2", type: "O", label: "Z", x: 800, y: 600 }
            ],
            defense: [ { id: "d1", type: "D", label: "M", x: 600, y: 540 } ],
            routes: [
                { id: "r1", ownerId: "wr1", ownerType: "O", type: "draw", color: "#f59e0b", points: [{x: 400, y: 600}, {x: 450, y: 550}, {x: 800, y: 550}] },
                { id: "r2", ownerId: "wr2", ownerType: "O", type: "draw", color: "#f59e0b", points: [{x: 800, y: 600}, {x: 750, y: 560}, {x: 400, y: 560}] }
            ],
            chalkTalkHistory: [
                { role: 'user', text: 'Why does this work against Cover 3?' },
                { role: 'model', text: 'Because the dual crossing routes force the curl/flat defenders to squeeze inside, opening up the sidelines for the RB wheel.' }
            ],
            aiAnalysis: { formation: "Custom", summary: "Demo Mesh Concept" }
        });
